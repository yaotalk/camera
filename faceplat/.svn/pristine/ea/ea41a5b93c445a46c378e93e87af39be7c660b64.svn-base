...
public class Swagger2 {
	...
	public Docket createRestApi() {
		...
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).useDefaultResponseMessages(false)
				...
				.globalResponseMessage(RequestMethod.POST, responses).select()
				.apis(RequestHandlerSelectors.basePackage("com.minivision.faceplat")).paths(PathSelectors.any()).build();
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("小视人脸服务平台").description("小视人脸服务 RESTful APIs")
				// .termsOfServiceUrl("http:///")
				.contact(new Contact("PanXinmiao", null, "panxinmiao@minivision.com.cn")).version("1.0").build();
	}
