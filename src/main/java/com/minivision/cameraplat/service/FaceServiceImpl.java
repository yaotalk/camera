...
import com.minivision.cameraplat.faceplat.result.detect.DetectedFace.Rectangle;
...
import com.minivision.cameraplat.util.ImageUtils;
...
public class FaceServiceImpl implements FaceService {
  ...
  private void save(Face face, String faceToken, String facesetToken, byte[] imgData, String fileType) throws ServiceException{
   ...
   */
  @Override
  public Face save(Face face, String facesetToken, byte[] imgData, String fileType)
    ...
    @Override public FaceResult save(FaceParam param) throws ServiceException, IOException {
        FaceResult faceResult = new FaceResult();
        Face face  = new Face();
        face.setName(param.getName());
        face.setIdCard(param.getIdCard());
        face.setSex(param.getSex());
        face.setPhoneNumber(param.getPhoneNumber());
        save(face,param.getFacesetToken(),param.getImgFile().getBytes(),".jpg");
        faceResult.setFaceToken(face.getId());
        faceResult.setImgPath(face.getImgpath());
        return faceResult;
    }

    @Override public FaceUpdateResult update(FaceUpdateParam faceUpdateParam)
        throws ServiceException, IOException {
      try {
          Assert.notNull(faceUpdateParam.getFacesetToken(),"facesetToken must not be null");
          Assert.notNull(faceUpdateParam.getFaceToken(),"faceToken must not be null");
          Face oldFace = this.find(faceUpdateParam.getFaceToken());
          FaceUpdateResult faceUpdateResult = new FaceUpdateResult();
          if(oldFace != null) {
              String name = Optional.ofNullable(faceUpdateParam.getName()).orElse(oldFace.getName());
              String idCard =  Optional.ofNullable(faceUpdateParam.getIdCard()).orElse(oldFace.getIdCard());
              String sex = Optional.ofNullable(faceUpdateParam.getSex()).orElse(oldFace.getSex());
              String phoneNumber = Optional.ofNullable(faceUpdateParam.getPhoneNumber()).orElse(oldFace.getPhoneNumber());
              if (faceUpdateParam.getImgFile() != null) {
                  Face face = new Face(name, idCard,sex,phoneNumber);
                  save(face, faceUpdateParam.getFacesetToken(), faceUpdateParam.getImgFile().getBytes(),
                      ".jpg");
                  logger.warn("success to update face {} from facePlat,new faceToken is {},imgPath is {}",faceUpdateParam.getFaceToken(),face.getId(),face.getImgpath());
                  RemoveFaceResult removeFaceResult = this.delete(faceUpdateParam.getFacesetToken(),
                      faceUpdateParam.getFaceToken());
                  if (removeFaceResult.getFailureDetail() == null) {
                      executor.submit(() -> {
                          List<MonitorRecord> monitorRecords = monitorRecordRepository.findByFace(oldFace);
                          monitorRecords.forEach(monitorRecord -> {
                              monitorRecord.setFace(face);
                              monitorRecordRepository.save(monitorRecord);
                          });
                      });
                      faceUpdateResult.setFaceToken(face.getId());
                      faceUpdateResult.setImgPath(face.getImgpath());
                      return faceUpdateResult;
                  } else {
                      logger.error("remove face{} from faceSet{} failed,cause {}", faceUpdateParam.getFaceToken(),
                          faceUpdateParam.getFacesetToken(),removeFaceResult.getFailureDetail().toArray());
                      throw new ServiceException(removeFaceResult.getFailureDetail().toString());
                  }
              }
              else{
                    oldFace.setName(name);
                    oldFace.setIdCard(idCard);
                    oldFace.setSex(sex);
                    oldFace.setPhoneNumber(phoneNumber);
                    faceRepository.save(oldFace);
                    faceUpdateResult.setFaceToken(oldFace.getId());
                    faceUpdateResult.setImgPath(oldFace.getImgpath());
                    return faceUpdateResult;
              }
          }
          else{
              logger.error("update face failed, face [{}] not exist ",faceUpdateParam.getFaceToken());
              throw  new ServiceException("facetoken not exist!");
          }
      }catch (Exception e){
          logger.error("update face failed,catch runtime exception {}",e.getMessage());
   ...
   * @param faceTokens
   */
  public RemoveFaceResult delete(String facesetToken, String faceTokens) throws ServiceException {
      try {
          Assert.notNull(facesetToken,"facesetToken must not be null !");
          Assert.notNull(faceTokens,"faceTokens must not be null !");
          RemoveFaceResult removeFaceResult = facePlatClient.removeFace(facesetToken, faceTokens);
          logger.warn("removeFaceResult from faceplat is {}",removeFaceResult);
          List<String> delTokens;
          if(removeFaceResult.getFailureDetail() != null) {
              List<String> failTokens =
                  removeFaceResult.getFailureDetail().stream().map(FailureDetail::getFaceToken).collect(Collectors.toList());
              delTokens =
                  Arrays.stream(faceTokens.split(",")).filter(o -> !failTokens.contains(o)).collect(Collectors.toList());
          }else{
              delTokens  = Arrays.asList(faceTokens.split(","));
          }
          for (String faceToken : delTokens) {
          ...
          faceRepository.deleteByIdIn(delTokens);
//          logger.warn("delete  success tokens {} from facesetToken {}", delTokens,facesetToken);
          return removeFaceResult;
      }
      catch (FacePlatException e){
          logger.error("remove face{} from faceset{} failed ,catch runtime exception {}",e.getMessage());
          throw new ServiceException(e);
      }
      catch (Exception e){
          logger.error("remove face{} from faceset{} failed ,catch unknown exception {}",e.getMessage());
  ...
  @Override public Page<Face> findByPage(PageParam pageParam,String facesetToken) {
...
public List<Face> getRepeatFaceBySetToken(String faceSetToken,PageParam pageParam) {
	String sql = "select t1.id,t1.`name`,t1.sex,t1.id_card,t1.employee_id,t1.pic_size,t1.pic_md5,t1.imgpath,t1.phone_number,t1.faceset_id,t1.create_time from `face` as t1,"
    ...
    @Override public List<Face> findFaceByFaceTokens(List<String> faceTokens) {
        return faceRepository.findByIdIn(faceTokens);
    }

    @Override
public int getRepeatFaceCoutBySetToken(String faceSetToken) {
	String sql = "select count(1) from `face` as t1,"
			+" (select count(1),ta.pic_md5 from  `face` as ta,"
			+" (SELECT count(1),t.pic_size FROM `face` AS T where T.faceset_id=:faceSetToken  and t.pic_size is not null and t.pic_md5 is not null"
			+" group by t.pic_size HAVING count(1)>1) as tb"
			+" where ta.pic_size=tb.pic_size  and ta.faceset_id=:faceSetToken group by  pic_md5 having count(1)>1) as t2 where t1.pic_md5=t2.pic_md5 "
...
public int checkIfHasNotDetected(String faceSetToken) {
	String sql = "select count(1) from face as t where (t.pic_md5 is null or t.pic_size is null) and t.faceset_id=:faceSetToken";
	Query query = manager.createNativeQuery(sql);
	query.setParameter("faceSetToken", faceSetToken);
	return ((BigInteger) query.getSingleResult()).intValue();
}
...
public List<Face> getNotDetectedData(String faceSetToken,PageParam pageParam) {
	String sql = "select t.id,t.`name`,t.sex,t.id_card,t.employee_id,t.pic_size,t.pic_md5,t.imgpath,t.phone_number,t.faceset_id"
...
public List<Face> getToProcess(String taskId, PageParam pageParam,boolean ifOrderBy) {
	String sql = "select ta.id,ta.`name`,ta.sex,ta.id_card,ta.employee_id,ta.pic_size,"
			+ "ta.pic_md5,ta.imgpath,ta.phone_number,ta.faceset_id,tb.is_store,ta.create_time from face as "
...
@Override
public List<Face> getToProcess(String taskId, PageParam pageParam,boolean ifOrderBy,boolean isIncludeStore) {
	String sql = "select ta.id,ta.`name`,ta.sex,ta.id_card,ta.employee_id,ta.pic_size,"
			+ "ta.pic_md5,ta.imgpath,ta.phone_number,ta.faceset_id,tb.is_store from face as "
			+ "ta right join face_tmp as tb on (ta.id=tb.pic_id) where tb.task_id=:taskId";
	String searchName = pageParam.getSearch();
	if(StringUtils.isNotBlank(searchName)){
		sql = sql + " and (ta.`name` like :searchName or ta.id_card like :searchName or ta.employee_id like :searchName or ta.pic_md5 like :searchName)";
	}
	if(ifOrderBy){
		sql = sql + " order by ta.pic_md5,tb.is_store desc";
	}
	...
	for(int i=0;i<resultList.size();i++){
		...
		faceList[i].setPhoneNumber((String) resultList.get(i)[8]);
	}
...
public void saveTmp(List<FaceTmp> faceTmpList) {
	String sql = "update face_tmp as t set t.is_store=:isStore where t.task_id=:taskId  and t.pic_id=:picId"; 
	Query query = manager.createNativeQuery(sql);
	for (FaceTmp faceTmp : faceTmpList) {
...
public boolean isCanSaveStore(FaceTmp faceTmp){
	String sql = "select count(1) from face as t1, (select tf.pic_md5 from face as tf where "
			...
			+ "t3.task_id=:taskId and t3.pic_id=t1.id and "
			+ "t3.pic_id!=:picId and t3.is_store=1";
